...
void Widget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (game->gameStatus==unknownStatus)
    {
        if (mousePosPvP&&event->button()==Qt::LeftButton) initPvPGame();
        if (mousePosPvE&&event->button()==Qt::LeftButton) initPvEGame();
        mouseClickPvP=false;
        mouseClickPvE=false;
    }

    //落子
    if (game->gameStatus==PLAYING&&rightMousePos&&event->button()==Qt::LeftButton)
    {
    ...
    //cheat
    if (game->gameStatus==PLAYING&&!game->AITurn&&event->button()==Qt::RightButton&&game->cheatMode)
    {
        game->cheat(mousePosRow,mousePosCol);
        update();
    }
}
//压下鼠标
void Widget::mousePressEvent(QMouseEvent *event)
{
    ...
    if (game->gameStatus==unknownStatus)
    {
        if (mousePosPvP) mouseClickPvP=true;
        if (mousePosPvE) mouseClickPvE=true;
    }
    update();
}
...
void Widget::keyPressEvent(QKeyEvent *event){
    ...
    {
        // 按键重复时不做处理
        if(event->isAutoRepeat()||game->gameMode!=PvE) return;
